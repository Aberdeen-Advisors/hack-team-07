...
window.BRIDGE_DATA =  {
  ...
  items: [
    /* — Alex Rivera (@arivera) — */
    {id:'a1', kind:'Action', urg:'High', title:'Send the approved Finance & Billing deck to attendees ahead of the call', owner:'AR', due:'2026-08-10', status:'open', deliv:{mid:'fincyc'}, note:'Deck reviewed and approved. Requested to go out before the call, by end of day if possible.', src:{kind:'slack', url:'#'}, verification:{status:'pending',decidedBy:null,decidedAt:null,note:'Captured from a source; no person has checked it.'}},
    {id:'a2', kind:'Action', urg:'High', title:'Chase the partner team on outstanding contract signatures', owner:'AR', due:'2026-08-10', status:'open', note:'No-show at the 8/3 steering call. Overdue from 8/4, in progress.', verification:{status:'verified',decidedBy:'seeded',decidedAt:null,note:'Typed into the tracker by a person before Bridge existed.'}},
    {id:'a3', kind:'Action', urg:'High', title:'Finalize the Compliance V1 deck — add standards slides plus a decisions-needed slide', owner:'AR', due:'2026-08-10', status:'open', deliv:{mid:'compws'}, note:'Overdue from 8/4, in progress.', src:{kind:'transcript', docId:'comptrans'}, verification:{status:'pending',decidedBy:null,decidedAt:null,note:'Captured from a source; no person has checked it.'}},
    {id:'a4', kind:'Action', urg:'High', title:'Present the system build timeline at the 8/17 leadership deliverable review', owner:'AR', due:'2026-08-24', status:'open', note:'Sequence: individual meetings, deep dive, timeframes and dependencies, partner overlay, align, present.', src:{kind:'transcript', docId:'buildtrans'}, verification:{status:'pending',decidedBy:null,decidedAt:null,note:'Captured from a source; no person has checked it.'}},
    {id:'a5', kind:'Action', urg:'Med', title:'Send the consolidated project plan to the partner team for validation and discrepancy review', owner:'AR', due:'2026-08-24', status:'open', note:'From the 8/4 morning standup.', verification:{status:'verified',decidedBy:'seeded',decidedAt:null,note:'Typed into the tracker by a person before Bridge existed.'}},
    {id:'a6', kind:'Action', urg:'Med', title:'Confirm who owns taking the integration to the Change Advisory Board', owner:'AR', due:'2026-08-24', status:'open', deliv:{mid:'itmgr'}, note:'Raised as a deliverable gap on the delivery leadership call.', verification:{status:'verified',decidedBy:'seeded',decidedAt:null,note:'Typed into the tracker by a person before Bridge existed.'}},
    {id:'a7', kind:'Action', urg:'Med', title:'Add a support and ticketing section to the leadership status — who receives tickets after go-live', owner:'AR', due:'2026-08-24', status:'open', deliv:{mid:'itmgr'}, note:'Leadership status deliverable, still unowned.', verification:{status:'verified',decidedBy:'seeded',decidedAt:null,note:'Typed into the tracker by a person before Bridge existed.'}},
    ...
    {id:'a9', kind:'Action', urg:'Low', title:'Confirm the steering committee roster on the delivery side', owner:'AR', due:'2026-08-24', status:'open', note:'Is a technical executive sponsor needed beyond the current list?', verification:{status:'verified',decidedBy:'seeded',decidedAt:null,note:'Typed into the tracker by a person before Bridge existed.'}},
    {id:'a10', kind:'Action', urg:'High', title:'Build the steering deck timeline slide, date-anchored to an end-of-November phase 1 go-live', owner:'AR', due:'2026-08-24', status:'done', deliv:{mid:'steer'}, note:'Completed 2026-08-07. Go-live anchor confirmed as end of November 2026, superseding the prior early-November back-plan. v4 posted as a lean four-slide pack.', verification:{status:'verified',decidedBy:'seeded',decidedAt:null,note:'Typed into the tracker by a person before Bridge existed.'}},
    ...
    {id:'a13', kind:'Action', urg:'Med', title:'Confirm delegated-approval agreement legal review status and return the signed agreement', owner:'JC', due:'2026-08-24', status:'open', note:'Due end of week.', verification:{status:'verified',decidedBy:'seeded',decidedAt:null,note:'Typed into the tracker by a person before Bridge existed.'}},
    {id:'a14', kind:'Action', urg:'Med', title:'Roll the week of action-item updates into the steering Next Steps slide', owner:'JC', due:'2026-08-17', status:'open', deliv:{mid:'steer'}, note:'Folded into the per-workstream next-steps slide.', verification:{status:'verified',decidedBy:'seeded',decidedAt:null,note:'Typed into the tracker by a person before Bridge existed.'}},
    ...
    {id:'a20', kind:'Action', urg:'Med', title:'Share the combined interface script tracker with the partner team', owner:'SP', due:'2026-08-10', status:'open', deliv:{mid:'wqreview0812'}, note:'Due 8/12 ahead of the work queue review.', verification:{status:'verified',decidedBy:'seeded',decidedAt:null,note:'Typed into the tracker by a person before Bridge existed.'}},
    ...
    {id:'a28', kind:'Action', urg:'Med', title:'Update meeting dates on steering deck slide 3', owner:'MK', due:'2026-08-24', status:'done', deliv:{mid:'steer'}, note:'Completed 2026-08-07. Slide 3 stays as an overview; dates moved to the timeline slide.', verification:{status:'verified',decidedBy:'seeded',decidedAt:null,note:'Typed into the tracker by a person before Bridge existed.'}},
    ...
    {id:'r2', kind:'Risk', urg:'High', title:'Record load duplicate matching — roughly a third of 14,500 records read as duplicates', owner:'SP', sev:'R', reviewed:null, status:'open', note:'Proposed workaround is a provisional-record workflow: hold the load, work the duplicates, then finalize. Internal decision still pending.', src:{kind:'transcript', docId:'buildtrans'}, verification:{status:'pending',decidedBy:null,decidedAt:null,note:'Captured from a source; no person has checked it.'}},
    {id:'r3', kind:'Risk', urg:'High', title:'Onboarding timeline is the main risk to the end-of-November go-live', owner:'JC', sev:'R', reviewed:null, status:'open', note:'The review committee meets monthly. Nov 9 is more realistic than Oct 26, which pushes past the planned go-live date.', verification:{status:'verified',decidedBy:'seeded',decidedAt:null,note:'Typed into the tracker by a person before Bridge existed.'}},
    ...
    /* — DECISIONS — */
    {id:'d1', kind:'Decision', urg:'High', title:'Phase 1 go-live anchored to end of November 2026', owner:'Team', date:'Aug 7', note:'Confirmed on both sides. Supersedes the earlier early-November back-plan. All timeline slides now key off this date.', verification:{status:'verified',decidedBy:'seeded',decidedAt:null,note:'Typed into the tracker by a person before Bridge existed.'}},
    {id:'d2', kind:'Decision', urg:'High', title:'Delegated approval path is off — standard individual onboarding starts now', owner:'Team', date:'Aug 4', note:'With only 34 partner staff, the delegated agreement is not worth the wait. Individual onboarding begins immediately.', verification:{status:'verified',decidedBy:'seeded',decidedAt:null,note:'Typed into the tracker by a person before Bridge existed.'}},
